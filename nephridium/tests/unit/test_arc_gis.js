...
describe('buildUrl', () => {
  it('builds url for one week', () => {
    ...
    expect(result).to.equal(
      `https://a.arcgis.dataset.com/resource/abcd-efgh/0/query?where=REQUEST_DATE%20%3E%20CURRENT_TIMESTAMP%20-%20INTERVAL%20%277%27%20DAY&orderByFields=REQUEST_DATE%20DESC&outFields=*&f=json`,
    );
  });
  ...
  it('builds with additional params', () => {
    ...
    expect(result).to.equal(
      `https://a.arcgis.dataset.com/resource/abcd-efgh/0/query?where=REQUEST_DATE%20%3E%20CURRENT_TIMESTAMP%20-%20INTERVAL%20%277%27%20DAY+AND+STATUS%3D%27Open%27&orderByFields=REQUEST_DATE%20DESC&outFields=*&f=json`,
    );
  });
});
...
describe('buildSearchParams', () => {
  it('handles search params', () => {
    ...
    expect(result).to.equal(
      `+AND+STATUS%3D%27Open%27+AND+DISTRICT_COUNCIL%3D%277%27`,
    );
  });
});
